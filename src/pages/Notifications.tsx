    ...
    <div className="min-h-screen pb-20 animate-fade-in-up">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 sm:py-8">
        ...
        {filteredNotifications.length === 0 ? (
          <div className="bg-white dark:bg-black border-l-8 border-cyan-500 p-12 text-center shadow-lg">
            <Bell className="w-16 h-16 mx-auto mb-4 text-gray-400 dark:text-gray-600" />
            <p className="text-xl font-black uppercase text-black dark:text-white mb-2">NO NOTIFICATIONS</p>
            <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500">YOU'RE ALL CAUGHT UP!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredNotifications.map(n => (
              <div 
                key={n.id} 
                className={`bg-white dark:bg-black border-l-8 p-6 shadow-lg transition hover:bg-gray-50 dark:hover:bg-gray-900 ${
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={`w-12 h-12 flex items-center justify-center flex-shrink-0 ${
                    n.type === 'success' ? 'bg-green-500/20 text-green-600 dark:text-green-400' :
                    n.type === 'warning' ? 'bg-yellow-400/20 text-yellow-600 dark:text-yellow-400' :
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4 mb-2">
                      <h4 className="font-black uppercase text-black dark:text-white">{n.title}</h4>
                      {!n.read && (
                    ...
                    <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 mb-3">{n.message}</p>
                    <div className="flex items-center gap-4">
                      <p className="text-xs font-bold uppercase text-gray-500 dark:text-gray-600">
