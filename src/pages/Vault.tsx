      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white dark:bg-black border-l-4 border-cyan-500 p-4 shadow-lg">
            <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 mb-1">TOTAL DOCUMENTS</p>
            <p className="text-2xl font-black text-black dark:text-white">{totalDocs}</p>
          </div>
          <div className="bg-white dark:bg-black border-l-4 border-yellow-400 p-4 shadow-lg">
            <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 mb-1">STORAGE USED</p>
            <p className="text-2xl font-black text-black dark:text-white">{formatFileSize(totalSize)}</p>
          </div>
          <div className="bg-white dark:bg-black border-l-4 border-green-500 p-4 shadow-lg">
            <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 mb-1">ENCRYPTED</p>
            <p className="text-2xl font-black text-green-600 dark:text-green-400">{totalDocs}</p>
          </div>
          <div className="bg-white dark:bg-black border-l-4 border-gray-800 dark:border-white p-4 shadow-lg">
            <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 mb-1">LAST BACKUP</p>
            <p className="text-sm font-black text-black dark:text-white">2 HOURS AGO</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border-2 border-gray-300 dark:border-gray-800 bg-white dark:bg-black text-black dark:text-white placeholder-gray-500 focus:border-cyan-500 outline-none font-bold uppercase text-xs"
            />
          </div>
          <button 
          ...
          {Object.entries(VAULT_CATEGORIES).map(([key, category]) => {
            ...
            return (
              <div
                ...
                className={`bg-white dark:bg-black border-l-8 p-6 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-900 transition-all shadow-lg ${
                  ...
                  category.color === 'white' ? 'border-gray-800 dark:border-white' :
                  'border-gray-500'
                }`}
              >
                <div className="text-4xl mb-3">{category.icon}</div>
                <h3 className="text-xl font-black uppercase text-black dark:text-white mb-2">{category.label}</h3>
                <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 mb-3">
                  {docs.length} {docs.length === 1 ? 'DOCUMENT' : 'DOCUMENTS'}
                </p>
                <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-900 px-3 py-1 w-fit">
                  <FileText size={14} className="text-cyan-600 dark:text-cyan-400" />
                  <span className="font-black text-black dark:text-white text-sm">{docs.length}</span>
                </div>
              </div>
            );
          })}
        ...
        {selectedCategory && (
          <div className="bg-white dark:bg-black border-l-8 border-cyan-500 p-6 shadow-lg animate-fade-in">
            ...
            {(!groupedDocuments[selectedCategory] || groupedDocuments[selectedCategory].length === 0) ? (
              ...
              <div className="space-y-3">
                {groupedDocuments[selectedCategory].map(doc => (
                  <div key={doc.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 transition border-l-4 border-cyan-500">
                    <div className="flex items-center gap-4 flex-1">
                      <div className="w-12 h-12 bg-cyan-500/20 flex items-center justify-center text-cyan-600 dark:text-cyan-400">
                        <FileText size={24} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-black uppercase text-black dark:text-white truncate">{doc.title}</h4>
                        <div className="flex items-center gap-3 mt-1">
                          <p className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500">
        ...
        <div className="mt-8 bg-white dark:bg-black border-l-8 border-green-500 p-6 shadow-lg">
          <div className="flex items-start gap-4">
            <Shield className="text-green-600 dark:text-green-400 flex-shrink-0 mt-1" size={32} />
            <div>
              <h3 className="font-black uppercase text-black dark:text-white mb-2 text-xl">BANK-LEVEL SECURITY</h3>
              <ul className="text-xs font-bold uppercase text-gray-600 dark:text-gray-500 space-y-2">
