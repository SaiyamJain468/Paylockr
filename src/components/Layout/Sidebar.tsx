...
export const Sidebar: React.FC<SidebarProps> = ({ view, setView, handleLogout }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`w-64 bg-white dark:bg-black border-r-4 border-yellow-400 flex flex-col fixed h-full transition-transform duration-300 ${isMobileOpen ? 'translate-x-0 z-40' : '-translate-x-full md:translate-x-0 z-40'
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto" role="navigation">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 font-bold uppercase text-xs transition-all ${view === item.id
