...
export default function App() {
  ...
  const handleLoginSuccess = () => {
    ...
    if (storedData) {
      ...
      setToast({ msg: 'Login failed. Please try again.', type: 'error' });
      setView('LOGIN');
    }
  };
  ...
  // Bulk import — adds ALL transactions in ONE state update AND persists to data layer
  const addTransactions = (newTxns: Transaction[]) => {
    if (!newTxns.length) return;
    // Step 1: Persist to ALL_USER_DATA + localStorage FIRST
    // This ensures they survive any auth-state-change or re-render that reloads getUserData()
    const uid = session?.user?.id || 'saiyam';
    saveTransactions(uid, newTxns);

    // Step 2: Update React state with merged list from the now-updated data layer
    const freshData = getUserData(uid);
    setFinancialData(prev => prev ? ({
      ...prev,
    ...
    setToast({ msg: `✅ ${newTxns.length} transactions imported`, type: 'success' });
  };
    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-neutral-950' : 'bg-gray-100'}`}>
      ...
      <main className="md:ml-64 min-h-screen">
        ...
        <div className={view === 'DASHBOARD' ? '' : 'px-4 md:px-8 pb-8'}>
          ...
          {view === 'TRANSACTIONS' && <Transactions transactions={transactions} onAdd={addTransaction} onBulkAdd={addTransactions} onUpdate={(updated) => {
            setFinancialData(prev => prev ? ({
              ...prev,
